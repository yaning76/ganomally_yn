...
parser.add_argument("--experiment", default=r"../experiments/test_Ganomaly_SSPCAB2_M5", help="path to save experiments results")
...
parser.add_argument("--gen_pth", default=r"../experiments/train_Ganomaly_SSPCAB2_M5/gen_best.pth", help="pretrained model of gen")
parser.add_argument("--disc_pth", default=r"../experiments/train_Ganomaly_SSPCAB2_M5/disc_best.pth", help="pretrained model of disc")
...
#     print('splitImage(test_input, opt.imageSize)',(splitImage(test_input, opt.imageSize)).shape)# torch.Size([1, 3, 128, 128])
    ...
    ## inference
    with torch.no_grad():
#         outputs, latent_in, latent_out,out1,out1_,out2,out2_,out3,out3_ 
        outputs, latent_in, latent_out,_,_,_,_,_,_ = gen(test_inputs)
...
#         print('outputs.shape, latent_in.shape, latent_out.shape',outputs.shape, latent_in.shape, latent_out.shape)
    ...
    lcon=L_con(outputs, test_inputs).item()
    lenc=L_enc(latent_in, latent_out).item()
    total_loss.append(0.5*lcon+0.5*lenc)
