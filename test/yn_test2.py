...
parser.add_argument("--experiment", default=r"../experiments/test_SCADN2_16_1", help="path to save experiments results")
...
parser.add_argument("--gen_pth", default=r"../experiments/train_SCADN2_16_1/gen_best.pth", help="pretrained model of gen")
parser.add_argument("--disc_pth", default=r"../experiments/train_SCADN2_16_1/disc_best.pth", help="pretrained model of disc")
...
for i, (test_input, label, mask) in enumerate(tqdm_loader):
    ...
    for j in range(len(test_input)):
        test_img=test_input[j]
        test_img=test_img.to(device)
        with torch.no_grad():
            output= gen(test_img,mask)
            _, features_real = disc(mask)
            _, features_feak = disc(output)
#             torch.cat((mask, output, residule), dim=0)
            vutils.save_image(output, '{0}/{1}-{2}.png'.format(opt.experiment, i,j))
