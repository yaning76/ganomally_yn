...
parser.add_argument("--experiment", default=r"../experiments/test_Ganomaly_SSPCAB2_M_grid", help="path to save experiments results")
...
parser.add_argument("--gen_pth", default=r"../experiments/train_Ganomaly_SSPCAB2_M_grid/gen_best.pth", help="pretrained model of gen")
parser.add_argument("--disc_pth", default=r"../experiments/train_Ganomaly_SSPCAB2_M_grid/disc_best.pth", help="pretrained model of disc")
...
#     print('splitImage(test_input, opt.imageSize)',(splitImage(test_input, opt.imageSize)).shape)# torch.Size([1, 3, 128, 128])
    ...
    ## inference
    with torch.no_grad():
        outputs,_,_,_,_,_,_,_,_,_= gen(test_inputs)#################
...
#     enc_loss.append(L_enc(latent_in, latent_out).item())
    ############################################################
    score.append((L_con(outputs, test_inputs)).item())
