...
from models.Unet4_w1_SSPCAB2 import NetG,NetD
from models.SSIM import SSIM

parser = argparse.ArgumentParser()
parser.add_argument("--experiment", default=r"../experiments/train_Unet4_w1_SSPCAB2_grid_ssim", help="path to save experiments results")
parser.add_argument("--dataset", default="grid", help="mnist")
parser.add_argument('--dataroot', default=r"../data/", help='path to dataset')
parser.add_argument("--n_epoches", type=int, default=350, help="number of epoches of training")
...
parser.add_argument("--gen_pth", default=r"../experiments/train_ml_memAE_sc_bottle/gen.pth", help="pretrained model of gen")
parser.add_argument("--disc_pth", default=r"../experiments/train_ml_memAE_sc_bottle/disc.pth", help="pretrained model of disc")
...
with tqdm.tqdm(range(opt.n_epoches)) as t:
    for e in t:
        ...
        L_tr_entropy_loss_func2_ = 0.0
        L_tr_entropy_loss_func3_ = 0.0
        L_tr_entropy_loss_func1_ = 0.0
        L_tr_entropy_loss_func1 = 0.0
        L_tr_entropy_loss_func0 = 0.0
        L_sspcab_epoch_loss1 = 0.0#改
        L_sspcab_epoch_loss2 = 0.0#改
#         L_tr_entropy_loss_func0_ = 0.0
...
#             print(D_real.shape,label_real.shape)
            ...
            outputs,_,_,_,_,_,_,_,_,_,_,_= gen(inputs)
#             print(outputs.shape)
...
########################################################
        ...
        #torch.Size([32, 3, 128, 128]) torch.Size([32, 100, 1, 1]) torch.Size([32, 100, 1, 1])
            outputs,out1,out1_,out2,out2_, att3,att3_,att2,att2_,att1,att1_,att0= gen(inputs)
...
#             print(feature_fake.shape)
            ...
            sspcab_loss1=L_sspcab1(out1,out1_)#改
            sspcab_loss2=L_sspcab2(out2,out2_)#改
            entropy_loss2_ = tr_entropy_loss_func2_(att2_)
            entropy_loss2 = tr_entropy_loss_func2(att2)
            entropy_loss3_ = tr_entropy_loss_func3_(att3_)
            entropy_loss3 = tr_entropy_loss_func3(att3)
            entropy_loss1_ = tr_entropy_loss_func1_(att1_)
            entropy_loss1 = tr_entropy_loss_func1(att1)
            entropy_loss0 = tr_entropy_loss_func0(att0)
#             entropy_loss0_ = tr_entropy_loss_func0_(att0_)
...
#             l_en.append(enc_loss)
        ...
        opt.lambda_con *(con_loss+ssim_loss) + \
        opt.lambda_lat*enc_loss+ \
        entropy_loss_weight*(entropy_loss2+entropy_loss3+entropy_loss2_+entropy_loss3_+
                             entropy_loss1+entropy_loss1_+entropy_loss0)+0.01*(sspcab_loss1+sspcab_loss2)#改
            ...
            L_tr_entropy_loss_func2_+=entropy_loss2_.item() * batch_size
            L_tr_entropy_loss_func2+=entropy_loss2.item() * batch_size
            L_tr_entropy_loss_func3_+=entropy_loss3_.item() * batch_size
            L_tr_entropy_loss_func3+=entropy_loss3.item() * batch_size
            L_tr_entropy_loss_func1_+=entropy_loss1_.item() * batch_size
            L_tr_entropy_loss_func1+=entropy_loss1.item() * batch_size
            L_tr_entropy_loss_func0+=entropy_loss0.item() * batch_size
            L_sspcab_epoch_loss1 += sspcab_loss1.item() * batch_size#改
            L_sspcab_epoch_loss2 += sspcab_loss2.item() * batch_size#改
#             L_tr_entropy_loss_func0_+=entropy_loss0_.item() * batch_size
...
#         print(enc_loss)
        ...
        L_ssim/= opt.dataSize
        L_tr_entropy_loss_func2_ /= opt.dataSize
        L_tr_entropy_loss_func2 /= opt.dataSize
        L_tr_entropy_loss_func3_ /= opt.dataSize
        L_tr_entropy_loss_func3 /= opt.dataSize
        L_tr_entropy_loss_func1_ /= opt.dataSize
        L_tr_entropy_loss_func1 /= opt.dataSize
        L_tr_entropy_loss_func0 /= opt.dataSize
        L_sspcab_epoch_loss1 /= opt.dataSize#改
        L_sspcab_epoch_loss2 /= opt.dataSize#改
#         L_tr_entropy_loss_func0_ /= opt.dataSize
        
        L_total_epoch_loss /= opt.dataSize
        disc_epoch_loss /= opt.dataSize
        
        
        print('L_adv:{},L_con:{},L_lat_epoch_loss:{},L_tr_entropy_loss_func:{},L_total:{},disc_loss{}'.format(L_adv_gen_epoch_loss,L_con_epoch_loss,L_lat_epoch_loss,L_tr_entropy_loss_func2,L_total_epoch_loss,disc_epoch_loss))
