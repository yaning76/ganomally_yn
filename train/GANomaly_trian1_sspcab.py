...
parser.add_argument("--experiment", default=r"../experiments/train_Ganomaly_SSPCAB_grid", help="path to save experiments results")
...
########################################################
        ...
        #torch.Size([32, 3, 128, 128]) torch.Size([32, 100, 1, 1]) torch.Size([32, 100, 1, 1])
            ...
            outputs, latent_in, latent_out,out3,out3_ = gen(inputs)
...
#             print('latent_out, latent_in',latent_out.shape, latent_in.shape)
            ...
            sspcab_loss=L_sspcab(out3,out3_)#改
        
            b,c,h,w=inputs.shape
#             print(enc_loss/b)
            l_en.append(enc_loss)
    
            l_sspcab.append(sspcab_loss)#改
        
            total_loss = opt.lambda_adv * adv_loss + \
                        opt.lambda_con * con_loss + \
                        opt.lambda_enc * enc_loss+0.01*sspcab_loss
            total_loss.backward()
            ...
            L_sspcab_epoch_loss += sspcab_loss.item() * batch_size#改
...
#         print(enc_loss)
        ...
        L_sspcab_epoch_loss /= opt.dataSize#改
