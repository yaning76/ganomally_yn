...
from models.Ganomaly_SSPCAB2_M import NetG,NetD

parser = argparse.ArgumentParser()
parser.add_argument("--experiment", default=r"../experiments/train_Ganomaly_SSPCAB2_M_grid", help="path to save experiments results")
parser.add_argument("--dataset", default="grid", help="mnist")
parser.add_argument('--dataroot', default=r"../data/", help='path to dataset')
parser.add_argument("--n_epoches", type=int, default=300, help="number of epoches of training")
...
tr_entropy_loss_func_1 = EntropyLossEncap().to(device)

tr_entropy_loss_func_2 = EntropyLossEncap().to(device)
tr_entropy_loss_func_3 = EntropyLossEncap().to(device)
tolloss=10000
...
with tqdm.tqdm(range(opt.n_epoches)) as t:
    for e in t:
        ...
        L_sspcab_epoch_loss3 = 0.0#改
        L_tr_entropy_loss_func_1 = 0.0
        
        L_tr_entropy_loss_func_2 = 0.0
        L_tr_entropy_loss_func_3 = 0.0
...
#             print(D_real.shape,label_real.shape)
            ...
            outputs,_,_,_,_,_,_,_,_,_= gen(inputs)
#             print(outputs.shape)
...
########################################################
        ...
        #torch.Size([32, 3, 128, 128]) torch.Size([32, 100, 1, 1]) torch.Size([32, 100, 1, 1])
            outputs, output_s1_,output_s1,output_s2_,output_s2,output_s3_,output_s3,att1,att2,att3,= gen(inputs)
...
#             print(feature_fake.shape)
            ...
            entropy_loss1 = tr_entropy_loss_func_1(att1)
            
            entropy_loss2 = tr_entropy_loss_func_2(att2)
            entropy_loss3 = tr_entropy_loss_func_3(att3)
            sspcab_loss1=L_sspcab1(output_s1_,output_s1)#改
            sspcab_loss2=L_sspcab2(output_s2_,output_s2)#改
            sspcab_loss3=L_sspcab3(output_s3_,output_s3)#改
#             enc_loss = L_enc(latent_out, latent_in)
            b,c,h,w=inputs.shape
#             print(enc_loss/b)
#             l_en.append(enc_loss)
            
            total_loss = adv_loss +con_loss + entropy_loss_weight*(entropy_loss1+entropy_loss2+entropy_loss3)+0.01*(sspcab_loss1+sspcab_loss2+sspcab_loss3)#改
            total_loss.backward()
            L_adv_epoch_loss += adv_loss.item() * batch_size
            L_con_epoch_loss += con_loss.item() * batch_size
            L_tr_entropy_loss_func_1+=entropy_loss1.item() * batch_size
            
            L_tr_entropy_loss_func_2+=entropy_loss2.item() * batch_size
            L_tr_entropy_loss_func_3+=entropy_loss3.item() * batch_size
            L_sspcab_epoch_loss1 += sspcab_loss1.item() * batch_size#改
            L_sspcab_epoch_loss2 += sspcab_loss2.item() * batch_size#改
            L_sspcab_epoch_loss3 += sspcab_loss3.item() * batch_size#改
...
#         print(enc_loss)
        ...
        L_tr_entropy_loss_func_1 /= opt.dataSize
        
        L_tr_entropy_loss_func_2 /= opt.dataSize
        L_tr_entropy_loss_func_3 /= opt.dataSize
        L_sspcab_epoch_loss1 /= opt.dataSize#改
        L_sspcab_epoch_loss2 /= opt.dataSize#改
        L_sspcab_epoch_loss3 /= opt.dataSize#改
        L_total_epoch_loss /= opt.dataSize
        disc_epoch_loss /= opt.dataSize
        print('L_adv:{},L_con:{},L_tr_entropy_loss_func:{},L_total:{},disc_loss{}'.format(L_adv_epoch_loss,L_con_epoch_loss,L_tr_entropy_loss_func_2,L_total_epoch_loss,disc_epoch_loss))
