...
from models.Ganomaly_MemAE5 import NetG,NetD

parser = argparse.ArgumentParser()
parser.add_argument("--experiment", default=r"../experiments/train_memae5_grid", help="path to save experiments results")
...
tr_entropy_loss_func_0 = EntropyLossEncap().to(device)
...
with tqdm.tqdm(range(opt.n_epoches)) as t:
    for e in t:
        ...
        L_tr_entropy_loss_func = 0.0
        L_tr_entropy_loss_func_0 = 0.0
...
########################################################
        ...
        #torch.Size([32, 3, 128, 128]) torch.Size([32, 100, 1, 1]) torch.Size([32, 100, 1, 1])
            outputs,att,att0, att1,att2,att3,= gen(inputs)
...
#             print(feature_fake.shape)
            ...
            entropy_loss = tr_entropy_loss_func(att)
            entropy_loss0 = tr_entropy_loss_func_0(att0)
...
#             l_en.append(enc_loss)
            ...
            total_loss = adv_loss +con_loss + entropy_loss_weight*(entropy_loss+entropy_loss0+entropy_loss1+entropy_loss2+entropy_loss3)
            total_loss.backward()
            L_adv_epoch_loss += adv_loss.item() * batch_size
            L_con_epoch_loss += con_loss.item() * batch_size
            L_tr_entropy_loss_func+=entropy_loss.item() * batch_size
            L_tr_entropy_loss_func_0+=entropy_loss0.item() * batch_size
...
#         print(enc_loss)
        ...
        L_tr_entropy_loss_func /= opt.dataSize
        L_tr_entropy_loss_func_0 /= opt.dataSize
