...
#         self.conv2 = double_conv(base_channels*16,base_channels*8)
        self.conv1 = double_conv(base_channels*16,base_channels*8)
        self.mem_rep1 = MemModule(mem_dim=mem_dim, fea_dim=base_channels*8, shrink_thres =shrink_thres)
        
        self.conv2= double_conv(base_channels*8,base_channels*4)
        self.mem_rep2 = MemModule(mem_dim=mem_dim, fea_dim=base_channels*4, shrink_thres =shrink_thres)
        
        self.conv3= double_conv(base_channels*4,base_channels*2)
        self.mem_rep3 = MemModule(mem_dim=mem_dim, fea_dim=base_channels*2, shrink_thres =shrink_thres)
        
        self.conv4= double_conv(base_channels*2,base_channels)
        self.mem_rep4 = MemModule(mem_dim=mem_dim, fea_dim=base_channels, shrink_thres =shrink_thres)
...
#Encoder
        ...
        x=self.pyramid0_D(out3)
        x = torch.cat([x, out2], dim=1)
        x=self.conv1(x)
        res_mem1= self.mem_rep1(x)
        f1 = res_mem1['output']
        att1 = res_mem1['att']
        
        x=self.pyramid1_D(f1)
        x = torch.cat([x, out1], dim=1)
        x=self.conv2(x)
        res_mem2= self.mem_rep2(x)
        f2 = res_mem2['output']
        att2 = res_mem2['att']
        
        x=self.pyramid2_D(f2)
        x = torch.cat([x, out0], dim=1)
        x=self.conv3(x)
        res_mem3 = self.mem_rep3(x)
        f3 = res_mem3['output']
        att3 = res_mem3['att']
        
        x=self.pyramid3_D(f3)
        x = torch.cat([x, out], dim=1)
        x=self.conv4(x)
        res_mem4 = self.mem_rep4(x)
        f4 = res_mem4['output']
        att4 = res_mem4['att']
        
        x=self.pyramid4_D(f4)

        return x,att1,att2,att3,att4
...
#         opt.imageSize, opt.nz, opt.nc, opt.ngf, opt.ngpu, opt.n_extra_layers
    ...
    def forward(self, x):
        gen_imag,att1,att2,att3,att4= self.encoder1(x)
        return gen_imag,att1,att2,att3,att4
...
class NetD(nn.Module):
    def __init__(self, opt):
        ...
        self.model = nn.Sequential(
            nn.Conv2d(opt.nc, ngf, 4, 2, 1, bias=False),#64
