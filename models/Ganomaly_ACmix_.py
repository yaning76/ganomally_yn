...
class Encoder_(nn.Module):
    ...
    def __init__(self, imageSize=128, nz=100, nc=3, ngf=64, ngpu=1, n_extra_layers=0, add_final_conv=True):
        ...
        self.pyramid3_ = nn.Sequential(#1,512,8,8->1,1024,4,4
            nn.Conv2d(base_channels*8, base_channels*16, 4, 2, 1, bias=False),
            nn.BatchNorm2d(base_channels*16),
            nn.LeakyReLU(0.2, inplace=True),
        )
...
#         self.add_final_conv=add_final_conv    
    def forward(self, input):
        ...
        output1=self.acmix1(output)
        
#         print(output.shape)
        
#         print(output.shape)
        output1=self.pyramid3_(output1)
        output1=self.acmix2(output1)
#         x.append(output)
...
#         self.acmix1=ACmix(in_planes=512, out_planes=512, kernel_att=7, head=4, kernel_conv=7, stride=1, dilation=1)
...
#         self.acmix2=ACmix(in_planes=1024, out_planes=1024, kernel_att=7, head=4, kernel_conv=7, stride=1, dilation=1)
#         if add_final_conv:
        self.final_conv=nn.Conv2d(base_channels*16, nz, 4, 1, 0, bias=False)
#         self.add_final_conv=add_final_conv    
    def forward(self, input):
