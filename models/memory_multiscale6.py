...
# window 窗口比例 fea_dim=w/window*h/window
        ...
        self.mem_rep2_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*2*2*2, window=2,c_num=8,shrink_thres =shrink_thres)
        ############               
    ...
    ############ 
        self.mem_rep_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*2*2*16, window=16,c_num=8,shrink_thres =shrink_thres)
        
        
        self.pyramid0_D = up(base_channels*16, base_channels*8)#32
        self.pyramid1_D = up(base_channels*8*2, base_channels*4)#32
        self.pyramid2_D = up(base_channels*4*2, base_channels*2)#64
        self.pyramid3_D = up(base_channels*2*2, base_channels)#128
        
        self.pyramid4_D =nn.Conv2d(base_channels, nc, 1)
        
        
        self.E0 = double_conv(nc*2, base_channels)#64,256,256
        self.E1 = down(base_channels, base_channels*2)#128,128,128
        self.E2= down(base_channels*2, base_channels*4)#256,64,64
        self.E3 = down(base_channels*4, base_channels*8)#512,32,32
        self.E4 = down(base_channels*8, base_channels*16)#1024,16,16
        
        self.D0 = up(base_channels*16, base_channels*8)#32
        self.D1 = up(base_channels*8*2, base_channels*4)#32
        self.D2 = up(base_channels*4*2, base_channels*2)#64
        self.D3 = up(base_channels*2*2, base_channels)#128
        
        self.D4 =nn.Conv2d(base_channels, nc, 1)
...
#Decoder
        x=self.pyramid0_D(res2_0['output'])
        x = torch.cat([res1_0['output'],x], dim=1)
        x=self.pyramid1_D(x)
        x = torch.cat([res0_0['output'],x], dim=1)
        x=self.pyramid2_D(x)
        x = torch.cat([res_0['output'],x], dim=1)
        x=self.pyramid3_D(x)
        x=self.pyramid4_D(x)
        
        ###############
        x1=self.E0(torch.cat([x,input], dim=1))
        x1_0=self.E1(x1)
        x1_1=self.E2(x1_0)
        x1_2=self.E3(x1_1)
        x1_3=self.E4(x1_2)
        y=self.D0(x1_3)
        y=self.D1(torch.cat([x1_2,y], dim=1))
        y=self.D2(torch.cat([x1_1,y], dim=1))
        y=self.D3(torch.cat([x1_0,y], dim=1))
        y=self.D4(y)
...
#         opt.imageSize, opt.nz, opt.nc, opt.ngf, opt.ngpu, opt.n_extra_layers
    ...
    def forward(self, x):
        gen_imag,res,y= self.encoder1(x)
        return gen_imag,res,y
