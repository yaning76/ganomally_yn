...
#         x = torch.cat([ res2_0['output'],res2_1['output']], dim=1)
        x=res2_0['output']+res2_1['output']
        x=self.pyramid0_D(x)
        x_=res1_1['output']+res1_0['output']+res1_2['output']
        x = torch.cat([x_,x], dim=1)
        x=self.pyramid1_D(x)
        x_=res0_1['output']+res0_0['output']+res0_2['output']
        x = torch.cat([x_,x], dim=1)
        x=self.pyramid2_D(x)
        x_=res_0['output']+res_1['output']+res_2['output']
        x = torch.cat([x_,x], dim=1)
        x=self.pyramid3_D(x)
        x=self.pyramid4_D(x)
#         print(x.shape)
