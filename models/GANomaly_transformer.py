...
class Encoder(nn.Module):
    ...
    def __init__(self, imageSize, nz, nc, ngf, ngpu, n_extra_layers=0, add_final_conv=True):
        ...
        self.eca0_=eca_block(channel=32)
        ...
        self.eca0=eca_block(channel=ngf)
        ...
        self.eca1=eca_block(channel=128)
        self.pyramid1 = nn.Sequential(
            nn.Conv2d(128, 256, 4, 2, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.eca2=eca_block(channel=256)
        self.pyramid2 = nn.Sequential(
            nn.Conv2d(256, 512, 4, 2, 1, bias=False),
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.2, inplace=True),
        )
        self.eca3=eca_block(channel=512)
        self.pyramid3 = nn.Sequential(
            nn.Conv2d(512, 1024, 4, 2, 1, bias=False),
            nn.BatchNorm2d(1024),
            nn.LeakyReLU(0.2, inplace=True),
        )
#         self.eca4=eca_block(channel=1024)
...
#         x=[]
        ...
        output=self.eca0_(output)
        output=self.pyramid0_(output)
        output=self.eca0(output)
#         x.append(output)
        
        output=self.pyramid0(output)
        output=self.eca1(output)
#         x.append(output)
        
        output=self.pyramid1(output)
        output=self.eca2(output)
#         x.append(output)
        
        output=self.pyramid2(output)
        output=self.eca3(output)
#         x.append(output)
        
        output=self.pyramid3(output)
#         output=self.eca4(output)
# #         x.append(output)
...
#             nn.ReLU(True),
#         )
#         self.eca0_=eca_block(channel=1024)
        self.pyramid0 = nn.Sequential(
            nn.ConvTranspose2d(1024,512, 4, 2, 1, bias=False),
            nn.BatchNorm2d(512),
            nn.ReLU(True),
        )
        self.eca0=eca_block(channel=512)
        self.pyramid1 = nn.Sequential(
            nn.ConvTranspose2d(512,256, 4, 2, 1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(True),
        )
        self.eca1=eca_block(channel=256)
        ...
        self.eca2=eca_block(channel=128)
        ...
        self.eca3=eca_block(channel=64)
        self.pyramid4 = nn.Sequential(
            nn.ConvTranspose2d(64,32, 4, 2, 1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(True),
        )
        self.eca4=eca_block(channel=32)
...
#         input=self.eca0_(input)
        input=self.pyramid0(input)
        input=self.eca0(input)
        input=self.pyramid1(input)
        input=self.eca1(input)
        input=self.pyramid2(input)
        input=self.eca2(input)
        input=self.pyramid3(input)
        input=self.eca3(input)
        input=self.pyramid4(input)
        input=self.eca4(input)
