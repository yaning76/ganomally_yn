...
# window 窗口比例 fea_dim=w/window*h/window
        ...
        self.mem_rep2_1 = MemModule1_new(mem_dim=mem_dim, fea_dim=256, window=1,shrink_thres =shrink_thres1)
        ############               
        self.mem_rep1_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*4*4, window=4,c_num=8,shrink_thres =shrink_thres)
        self.mem_rep1_1 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*8*4, window=8,c_num=16,shrink_thres =shrink_thres1)
        self.mem_rep1_2 = MemModule1_new(mem_dim=mem_dim, fea_dim=1024,window=1, shrink_thres =shrink_thres1)
        ############                       
        self.mem_rep0_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*4*8, window=8,c_num=8,shrink_thres =shrink_thres)
        self.mem_rep0_1 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*4*8, window=16,c_num=32,shrink_thres =shrink_thres1)
    
        self.mem_rep0_2 = MemModule1_new(mem_dim=mem_dim, fea_dim=1024,window=2, shrink_thres =shrink_thres1)
    ############ 
        self.mem_rep_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*2*16, window=16,c_num=16,shrink_thres =shrink_thres)
        self.mem_rep_1 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*2*32, window=32,c_num=32,shrink_thres =shrink_thres1)
        
        self.mem_rep_2 = MemModule1_new(mem_dim=mem_dim, fea_dim=1024,window=4, shrink_thres =shrink_thres1)
...
#         x=f3+f3_
        ...
        x = torch.cat([res1_1['output'],res1_0['output'],res1_2['output'],x], dim=1)
        x=self.pyramid1_D(x)

        x = torch.cat([res0_1['output'],res0_0['output'],res0_2['output'],x], dim=1)
        x=self.pyramid2_D(x)
        
        x = torch.cat([res_0['output'],res_1['output'],res_2['output'],x], dim=1)
        x=self.pyramid3_D(x)
        x=self.pyramid4_D(x)
#         print(x.shape)
        return x,res
