...
# window 窗口比例 fea_dim=w/window*h/window
        ...
        self.mem_rep2_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*2*2, window=2,c_num=8,shrink_thres =shrink_thres)
        ############               
    ...
    ############ 
        self.mem_rep_0 = MemModule_window(mem_dim=mem_dim, fea_dim=base_channels*2*16, window=16,c_num=8,shrink_thres =shrink_thres)#64,128,128
...
#Encoder
        ...
        out=self.en_doubleConv4(res2_0['output'])
        
        out=self.up1(out)#512
        x=torch.cat([res1_0['output'],out], dim=1)
        out=self.de_doubleConv1(torch.cat([res1_0['output'],out], dim=1))
                               
        out=self.up2(out)
        out=self.de_doubleConv2(torch.cat([res0_0['output'],out], dim=1))
                                
        out=self.up3(out)
        out=self.de_doubleConv3(torch.cat([res_0['output'],out], dim=1)) 
